;(function ($) {
        ...
        var pmaxHeight = Math.round($(window).height() * 0.75);
        var pmaxWidth = Math.round($(window).width() * 0.75);
    ...
    // Alert
    $.CfRepAlert = function (message) {

                var _dialog = $('<div id="alertDialog" title="Alert!"></div>').dialog(_opts)
                                                        .css("max-width",_opts.maxWidth)
                                                        .css("max-height",_opts.maxHeight);
                _dialog.html(message);
                _dialog.dialog('open');
		};

    $.CfRepAlert_jqXHR = function (jqXHR) {

        var msg_text;
        var html_mark = "<!DOCTYPE HTML>";

        if (jqXHR.responseText.slice(0, html_mark.length).toUpperCase() == html_mark) {
            msg_text = jqXHR.responseText;
        }
        else {

            var r = $.parseJSON(jqXHR.responseText);
            if (r == null) {
                r = { ExceptionType: "", Message: "", StackTrace: "" };
            }

            msg_text = "";

            msg_text += '<table border="0" bordercolor="FFFFFF" cellspacing="0" >';

            msg_text += '<tr><td align="left" valign="top">status:</td><td>' + jqXHR.status + '</td></tr>';
            //msg_text += '<tr><td align="left" valign="top">statusText:</td><td>' + jqXHR.statusText + '</td></tr>';
            msg_text += '<tr><td align="left" valign="top">statusText:</td><td>' + ((errors[jqXHR.status] === undefined) ? jqXHR.statusText : errors[jqXHR.status]) + '</td></tr>';
            
            msg_text += '<tr><td align="left" valign="top"><br></td><td><br></td></tr>';

            msg_text += '<tr><td align="left" valign="top">ExceptionType:</td><td>' + r.ExceptionType + '</td></tr>';

            msg_text += '<tr><td align="left" valign="top"><br></td><td><br></td></tr>';

             msg_text += '<tr><td align="left" valign="top">Message:</td><td>' + r.ExceptionMessage + '</td></tr>';

            msg_text += '<tr><td align="left" valign="top"><br></td><td><br></td></tr>';

            //if (jqXHR.status != "500")
            //{
                msg_text += '<tr><td align="left" valign="top">StackTrace:</td><td>' + r.StackTrace.replace("\r\n", "<br>") + '</td></tr>';
            //}
            msg_text += '</table>';
        }
        var _dialog = $('<div id="alertDialog" title="Alert!"></div>').dialog(_opts).css("max-width",_opts.maxWidth);
        _dialog.html(msg_text);
        _dialog.dialog('open');
    };
